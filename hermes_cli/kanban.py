...
def _task_to_dict(t: kb.Task) -> dict[str, Any]:
    return {
        ...
        "observation": bool(t.observation),
    }
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    p_create.add_argument("--observation", action="store_true",
                          help="Create an external observation card: it "
                               "mirrors a turn run by an outside agent "
                               "(e.g. the Claude Code / Codex adapters), is "
                               "born running with no claim or worker run, "
                               "and is never dispatched. Close it with "
                               "'complete'; orphans auto-close as done "
                               "after the observation TTL. Incompatible "
                               "with worker execution options (--goal, "
                               "--max-retries, --parent, --workspace, ...).")
    ...
    p_delete = sub.add_parser(
        "delete",
        help="Permanently delete tasks",
    )
    p_delete.add_argument(
        "task_ids",
        nargs="+",
        help="Task ids to permanently delete",
    )
    ...
    try:
        with kb.connect_closing() as conn:
            task_id = kb.create_task(
                ...
                observation=bool(getattr(args, "observation", False)),
            )
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    with kb.connect_closing() as conn:
        ...
        # Same rationale for orphaned observation cards — boards without a
        # dispatcher still get them closed as done after the TTL.
        kb.expire_orphan_observations(conn)
        tasks = kb.list_tasks(
            conn,
            assignee=assignee,
            status=args.status,
...
def _cmd_delete(args: argparse.Namespace) -> int:
    """Permanently delete tasks via the explicit CLI verb."""
    failed: list[str] = []
    with kb.connect_closing() as conn:
        for tid in args.task_ids:
            if not kb.delete_task(conn, tid):
                failed.append(tid)
                print(f"cannot delete {tid} (task not found)", file=sys.stderr)
            else:
                print(f"Deleted {tid}")
    return 0 if not failed else 1
